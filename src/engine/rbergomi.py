...
class RBergomiEngine:
    def __init__(self, T, N, H, eta, rho, xi_0, S_0, r, q=0):
        self.T = T
        self.N = N
        self.dt = T / N
        self.H = H
    ...
    def simulate_cholesky(self, n_paths):
        ...
        V_start = v[:-1, :]
        log_returns = np.sqrt(V_start) * dZ + (self.r - self.q - 0.5 * V_start) * self.dt
    ...
    def _generate_hybrid_increments(self, n_paths):
        ...
        z_ortho = np.random.randn(self.N, half_paths)
        
        z1_full = np.concatenate([z1, -z1], axis=1)
        z2_full = np.concatenate([z2, -z2], axis=1)
        z_ortho_full = np.concatenate([z_ortho, -z_ortho], axis=1)
        ...
        return dW_vol, dZ_price, z_ortho_full 

    def _fft_convolution(self, dW_vol, z_ortho):
        """
        ...
        """
        alpha = self.H - 0.5 + 1e-6
        
        k_idx = np.arange(2, self.N + 1)
        b_k = ((k_idx**(alpha + 1) - (k_idx - 1)**(alpha + 1)) / (alpha + 1)) ** (1 / alpha)
        
        b = np.zeros(self.N)
        b[1:] = (b_k * self.dt) ** alpha
        ...
        w_sing = (self.dt ** alpha) / (alpha + 1)
        
        var_true = 1.0 / (2 * alpha + 1)
        var_proj = 1.0 / ((alpha + 1) ** 2)
        noise_amp = np.sqrt(var_true - var_proj) * (self.dt ** (alpha + 0.5))
        
        W_corr = w_sing * dW_vol + noise_amp * z_ortho
        
        W_tilde = np.sqrt(2 * self.H) * (W_tilde_fft + W_corr)
    ...
    def simulate_hybrid(self, n_paths):
        ...
        """
        dW_vol, dZ_price, z_ortho = self._generate_hybrid_increments(n_paths)
    
        W_tilde = self._fft_convolution(dW_vol, z_ortho)
    
        t = self.times[:, None]
        v = self.xi_0 * np.exp(self.eta * W_tilde - 0.5 * self.eta**2 * t**(2 * self.H))
    
        V_start = v[:-1, :]
        log_returns = np.sqrt(V_start) * dZ_price + (self.r - self.q - 0.5 * V_start) * self.dt
        cum_log_returns = np.cumsum(log_returns, axis=0)
